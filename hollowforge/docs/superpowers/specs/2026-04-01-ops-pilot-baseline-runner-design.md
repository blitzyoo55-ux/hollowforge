...
기본 command는 spec 단계에서 아래처럼 고정한다.
...
이 범위는 아래 baseline concerns를 직접 커버한다.

- sequence registry / provider resolution
- story planner catalog / route contract
- marketing / publishing handoff continuity
- sequence runtime adult default guardrail

이번 runner는 위 backend pytest slice를 기본 contract로 사용한다. 구현 중 임의로
더 넓히거나 줄이지 않는다.
...
baseline runner는 기존 smoke script의 전체 출력에 의존하지 않고, 아래 stable
labels만 파싱한다.
...
즉 log rendering에 필요한 metadata는 위 label contract에서만 추출한다.
...
timeout 관련 플래그는 이번 범위에 추가하지 않는다. 기본 timeout은 내부 상수로
고정한다.

- backend pytest: 120초
- adult provider resolution: 15초
- story planner smoke: 60초
- frontend `npm test`: 180초

timeout이 나면 해당 check는 `FAIL`로 기록하고, summary에는 `timeout after <n>s`
형태의 이유를 남긴다. 다른 checks는 계속 실행한다.
...
timeout behavior도 테스트 범위에 포함한다. 즉 적어도 하나의 unit test는
subprocess timeout이 `FAIL + timeout summary`로 변환되고, 다른 checks 실행을
막지 않는지 검증해야 한다.
