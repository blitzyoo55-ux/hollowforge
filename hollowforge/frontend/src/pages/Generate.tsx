...
export default function Generate() {
  ...
  const presetInitial = useMemo<Partial<GenerationCreate> | undefined>(() => {
    if (fromGenerationId || !presetId || !presets) {
      return undefined
    }
    const preset = presets.find((item: PresetResponse) => item.id === presetId)
    if (!preset) {
      return undefined
    }
  ...
  }, [fromGenerationId, presetId, presets])
  ...
  const generateMutation = useMutation({
    ...
    onError: (err) => {
      ...
      let message = 'Failed to start generation. Is the backend running?'
