"""Repository helpers for production hub verification run history."""
...
def _encode_stage_status(
    stage_status: dict[str, ProductionVerificationStageStatusResponse],
...
def _decode_stage_status_json(
    value: Any,
) -> dict[str, ProductionVerificationStageStatusResponse]:
    ...
    if isinstance(value, str):
        try:
            ...
            raise ValueError(
                "Invalid JSON object in production_verification_runs.stage_status_json"
            ) from exc
    else:
        parsed = value
    ...
    return {
        str(stage_name): ProductionVerificationStageStatusResponse.model_validate(
            stage_data
        )
...
def _run_response(row: dict[str, Any]) -> ProductionVerificationRunResponse:
    payload = dict(row)
    ...
    return ProductionVerificationRunResponse.model_validate(payload)


async def create_production_verification_run(
    ...
    return ProductionVerificationRunResponse(
        id=run_id,
...
async def _fetch_latest_run_for_mode(
    db,
    run_mode: str,
) -> ProductionVerificationRunResponse | None:
...
async def _fetch_recent_runs(
    db,
    limit: int,
) -> list[ProductionVerificationRunResponse]:
    cursor = await db.execute(
        """
        SELECT
            id,
...
async def get_production_verification_summary(
    limit: int = 10,
) -> ProductionVerificationSummaryResponse:
    async with get_db() as db:
        await db.execute("BEGIN")
        try:
            latest_smoke_only = await _fetch_latest_run_for_mode(db, "smoke_only")
            latest_suite = await _fetch_latest_run_for_mode(db, "suite")
            recent_runs = await _fetch_recent_runs(db, limit) if limit > 0 else []
            await db.commit()
        except Exception:
            await db.rollback()
            raise

    return ProductionVerificationSummaryResponse(
        latest_smoke_only=latest_smoke_only,
