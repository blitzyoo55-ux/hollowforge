...
_GUIDE_CACHE_TTL_SEC = 120.0
_GUIDE_GENERATION_SCAN_LIMIT = 500
_GUIDE_CACHE_LOCK = asyncio.Lock()
_GUIDE_CACHE: dict[str, tuple[float, dict[str, Any]]] = {}
...
def _guide_cache_key(checkpoint: str | None) -> str:
    return checkpoint or "__all__"


def _get_cached_guide(cache_key: str) -> dict[str, Any] | None:
    entry = _GUIDE_CACHE.get(cache_key)
    if entry is None:
        return None
...
def _set_cached_guide(cache_key: str, payload: dict[str, Any]) -> None:
    _GUIDE_CACHE[cache_key] = (time.monotonic() + _GUIDE_CACHE_TTL_SEC, payload)


def _get_cache_ttl_sec(cache_key: str) -> int:
    entry = _GUIDE_CACHE.get(cache_key)
    if entry is None:
        return 0
    expires_at, _ = entry
...
async def get_lora_guide(
    ...
    cache_key = _guide_cache_key(checkpoint)
    if not refresh:
        cached_payload = _get_cached_guide(cache_key)
    ...
    async with _GUIDE_CACHE_LOCK:
        ...
        for filename, profile in lora_profiles.items():
            ...
            category = str(
                profile.get("category") or (analysis.category if analysis else "style")
            )
            base_strength = _to_float(
                profile.get("default_strength"),
                analysis.default_strength if analysis else 0.7,
            )
            compatible = parse_compatible_checkpoints(
                profile.get("compatible_checkpoints")
            )
            ...
            runs = int(usage["runs"])
            avg_strength = round(usage["sum_strength"] / runs, 3) if runs > 0 else None
            ...
            high_strength = round(
                _clamp(base_strength + span, 0.2, positive_high_cap), 2
            )
            reverse_start = round(
            ...
            for checkpoint_name in compatible:
                ...
                if lora_arch != "Unknown" and cp_arch != "Unknown":
                    ...
                    reasons.append(
                        "아키텍처 메타데이터가 불완전하여 파일명/키 패턴 기반으로 추정됨"
                    )
                ...
                if (
                    avg_strength is not None
                    and low_strength <= abs(avg_strength) <= high_strength
                ):
                    score += 5.0
            ...
            fit_entries.sort(
                key=lambda item: (-item["score"], -item["runs"], item["checkpoint"])
            )

            if checkpoint:
                fit_entries = [
                    fit for fit in fit_entries if fit["checkpoint"] == checkpoint
                ]
                if not fit_entries:
                    continue
            ...
            strength_example_entries.append(
                {
                    ...
                    "total_abs_strength": (
                        picked["total_abs_strength"] if picked else None
                    ),
        ...
        if checkpoint:
            checkpoint_entries = [
                item for item in checkpoint_entries if item["name"] == checkpoint
            ]
        ...
        _set_cached_guide(cache_key, payload)
