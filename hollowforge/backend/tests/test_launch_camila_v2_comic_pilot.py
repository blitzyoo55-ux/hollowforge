...
def test_main_can_optionally_verify_full_layered_handoff_after_selection(
    monkeypatch,
    capsys,
) -> None:
    module = _load_module()
    ...
    responses: dict[tuple[str, str], object] = {
        ...
        ("POST", f"{base_url}/api/v1/comic/episodes/import-story-plan"): {
            "episode": {
                "id": "comic-ep-camila-v2-dry-run-1",
                "render_lane": "character_canon_v2",
                "series_style_id": "camila_pilot_v1",
                "character_series_binding_id": "camila_pilot_binding_v1",
            },
        ...
        ("POST", f"{base_url}/api/v1/comic/panels/panel-1/assets/asset-1/select"): {
            "id": "asset-1",
            "generation_id": "gen-1",
            "scene_panel_id": "panel-1",
            "storage_path": "images/comics/panel-1-selected.png",
        ...
        ("POST", f"{base_url}/api/v1/comic/panels/panel-2/assets/asset-2/select"): {
            "id": "asset-2",
            "generation_id": "gen-2",
            "scene_panel_id": "panel-2",
            "storage_path": "images/comics/panel-2-selected.png",
        ...
        ("POST", f"{base_url}/api/v1/comic/panels/panel-3/assets/asset-3/select"): {
            "id": "asset-3",
            "generation_id": "gen-3",
            "scene_panel_id": "panel-3",
            "storage_path": "images/comics/panel-3-selected.png",
        ...
        ("POST", f"{base_url}/api/v1/comic/panels/panel-4/assets/asset-4/select"): {
            "id": "asset-4",
            "generation_id": "gen-4",
            "scene_panel_id": "panel-4",
            "storage_path": "images/comics/panel-4-selected.png",
    ...
    def fake_run_production_dry_run(**kwargs):  # type: ignore[no-untyped-def]
        ...
        return {
            ...
            "page_count": 1,
            "export_zip_path": "comics/exports/camila-v2-dry-run.zip",
            "layered_manifest_path": "comics/manifests/camila-v2-dry-run/manifest.json",
            "handoff_validation_path": "comics/manifests/camila-v2-dry-run/handoff_validation.json",
            "hard_block_count": 0,
            "report_path": "comics/reports/camila-v2-dry-run.json",
        }

    monkeypatch.setattr(module, "_request_json", fake_request_json)
    monkeypatch.setattr(module, "_run_production_dry_run", fake_run_production_dry_run)
    monkeypatch.setattr(
        sys,
        "argv",
        [
    ...
    assert dry_run_calls[0]["episode_id"] == "comic-ep-camila-v2-dry-run-1"
    assert dry_run_calls[0]["layout_template_id"] == module.DEFAULT_LAYOUT_TEMPLATE_ID
    assert dry_run_calls[0]["manuscript_profile_id"] == module.DEFAULT_MANUSCRIPT_PROFILE_ID
    assert "dialogues_success: true" in captured.out
    assert "generated_dialogue_count: 8" in captured.out
    ...
    assert "export_zip_path: comics/exports/camila-v2-dry-run.zip" in captured.out
    assert (
        "layered_manifest_path: comics/manifests/camila-v2-dry-run/manifest.json"
        in captured.out
    )
    ...
    assert "dry_run_report_path: comics/reports/camila-v2-dry-run.json" in captured.out
    assert "overall_success: true" in captured.out
...
def test_main_scales_remote_render_poll_budget_by_candidate_count(
    monkeypatch,
    capsys,
) -> None:
    module = _load_module()
    ...
    responses: dict[tuple[str, str], object] = {
        ...
        ("POST", f"{base_url}/api/v1/comic/episodes/import-story-plan"): {
            "episode": {
                "id": "comic-ep-camila-v2-poll-budget-1",
                "render_lane": "character_canon_v2",
                "series_style_id": "camila_pilot_v1",
                "character_series_binding_id": "camila_pilot_binding_v1",
            },
                ...
                "render_assets": [
                    {
                        "id": "asset-1",
                        "generation_id": "gen-1",
                        "storage_path": "outputs/asset-1.png",
                        "quality_score": 0.74,
                    },
                    {
                        "id": "asset-2",
    ...
    captured_poll_attempts: list[int] = []
    ...
    def fake_poll_render_job_output_path(**kwargs):  # type: ignore[no-untyped-def]
        captured_poll_attempts.append(int(kwargs["poll_attempts"]))
        return [
            {
                "id": "render-job-1",
                "render_asset_id": "asset-1",
    ...
    monkeypatch.setattr(
        ...
        fake_poll_render_job_output_path,
    )
    monkeypatch.setattr(
        sys,
        "argv",
    ...
    assert module.main() == 0
    capsys.readouterr()
    assert captured_poll_attempts == [840]
